...
const selectHrefIfContains = (doc, tag, query) => {
  const results = [];
  doc.querySelectorAll(tag)?.forEach(function (e) {
    if (e.textContent.toLowerCase().includes(query)) {
...
const confParser = (doc) => {
  let results = [];
  doc.querySelectorAll(".toc-link")?.forEach(function (e) {
    results.push(e.href);
  });

  results = [...results, ...selectHrefIfContains(doc, "p", "proceedings")];
  return results;
};

const jourParser = (doc) => {
  return selectHrefIfContains(doc, "li", "volume");
};
...
export async function parseList(list) {
  ...
  const results = [...confParser(doc), ...jourParser(doc)];
  return results;
}
